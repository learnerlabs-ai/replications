...
> **Scope.** This protocol replays the product demonstrations under `demos/` (the fact demonstrations and the
> two-languages study) on your own key. The ten-skill study and the four-domain comparison are checked by
> re-scoring and figure regeneration instead; see [REPRODUCIBILITY.md](REPRODUCIBILITY.md).
...
The product turns the document into short factual statements as part of the teach and reports how
many it accepted (`fact_extraction` in the response, and `GET /v1/facts?learner_id=lrn_…`
afterwards). That count is not fixed: one of the two recorded runs of the handbook stored 15
statements and the other stored 60, because part of the step is model-driven and is not
reproducible. [`extraction/`](extraction/) describes the whole path and publishes the prompts.

## 2b. Teach individual facts (override-a-belief)

One demonstration does not go through document extraction. Each fact is installed directly, then
one training call writes all pending facts at once. This is also the route for any fact the
extraction step declined from a document: it accepts only atomic one-subject-one-value statements,
and in the recorded runs it accepted only 4 of the 9 override facts when they were offered as a
document, so that demonstration installs them by fact.

`teach-in-sequence` does the opposite. Each of its three lessons is a short document taught
through step 2a, one after another into the same learner, and the statements come out of the
extraction step like any other document.
