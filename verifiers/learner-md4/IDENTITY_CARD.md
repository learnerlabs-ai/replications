...
| Lineage | `v9d1f46605855` records `v631332af9956` as its `base_version` in the artifact manifest — a version-history pointer, not a weight lineage: the artifact was produced by a retrain of the learner on its then-current fact set. It is the demonstration's published end-state, and the manifest names the producing job as `job_id` |
| Battery checkpoint pin | `v9d1f46605855`, verified: the production resolver served exactly this artifact on the evaluation host, and a live probe through it answered a taught counterfactual correctly |
| Battery results | `../battery/CAPABILITY.md` (aggregate) and `../battery/TRAJECTORY.md` (every teach step) |
