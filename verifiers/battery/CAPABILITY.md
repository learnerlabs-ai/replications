...
*Published with the benchmark battery. All numbers below are final. An earlier version of this
page mislabeled the second learner "two-languages"; it is the override-a-belief learner
(`lrn_c5020a579689`). Every number is unchanged.*
...
five tasks to a shipping race when the evaluation host shut down (their aggregate scores survive in the receipt;
...
`__base__` (the base model, taught nothing). The five intermediate checkpoints in `TRAJECTORY.md`
belong to a different pair of learners (the sequence-trained ones documented there) and are
