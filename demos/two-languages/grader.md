...
are zero for that generation. This happened in 15 of the 80 continuations, and those zeros are in
the published means.
...
demonstration scored 267 words. Read the contamination result as a clean zero across every cell
...
language. The distinction is in the per-row records in `answers/generations.json`, where every generation
is kept in full.
...
Its loss is measured on the last eight 512-token windows of each corpus, held out of its training, and that
method is stated in the file. The learner's held-out loss is measured on two 256-token windows withheld from
each corpus, so the two are not the same measurement; read each method against its own earlier level.
