...
**The claim.** Nine facts from an invented planet, chosen so the untrained model would not hold
them. It got all nine wrong in the recorded run and served eight of them afterwards, and Earth
physics was unchanged. One of the nine turns out to be reachable by the base model on its own, so
the count that carries the claim is six of the eight it does not hold, under the strict re-grade;
the paragraphs below say which and why.
...
| `answers/session.json` | The recorded run: base answers, taught answers, controls. |
...
`grader.md` and marked on the demonstrations page. The displayed score counts it as a miss.
...
**One of the nine is a fact the base model can produce on its own.** In the recording below the
base answered the sky question by saying no such planet exists. Asked the same question again in a
later measurement, it answered "green" and attributed the planet to a film franchise. A fact the
base can reach unaided is not one this demonstration can claim to have taught, so the number to
read is **six of the eight facts the base does not hold**, under the strict re-grade the
demonstrations page uses (the fire-extinguisher answer counted wrong), or seven of eight as the
session file records it. Eight of nine is what the session file records, and every reading is in it.
