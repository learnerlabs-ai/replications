...
There is no word boundary, with one exception: an accepted answer that begins or ends with a digit
must not sit inside a longer number, so `41` does not match inside `141`. Re-scoring this recording
with and without that exception gives the same verdict on all 24 rows. That is a real difference from
the grader used in the `teach-a-document` folder and it is stated here because it changes what a
count means.
...
**It got one row wrong in this run, in our favour. The session file keeps that machine verdict; the
displayed score counts the row as a fail.**
...
The session file keeps the verdict as the grader produced it. The displayed score re-grades the row
to a fail; see Re-grade below.
...
oxygen. We re-graded that row to a fail, so the learner served 7 of the 9 taught facts, not the 8 of
9 the mechanical grader printed. One of the nine, the sky colour, is a fact the base model can reach
on its own, so the displayed score counts the eight it does not hold: 6 of 8 under this re-grade,
7 of 8 as the session file records it. The machine verdict is preserved on the row in the answers
file. The correction runs against us, and it was found by reading the answers, which
