...
not retained by the harness. The full prompts
...
| label | teach step |
|---|---|
| document-lessons-ck1 | after lesson A |
| document-lessons-ck2 | after lesson B |
| document-lessons-ck3 | after lesson C |
...
task files (a shipping race when the evaluation host shut down); their aggregate scores survive in the receipts.


The complete raw evaluator rows (full few-shot prompts, per-item log-probabilities, targets)
for the evaluated models are published as a release, tagged `battery-raw-samples`
(sha256s in the attached RAWS_MANIFEST.json). The slim rows here derive from
